...
class MNMarqueeView: UIView {
    ...
    @IBInspectable var textScrollSpeed: CGFloat = 50+20 {
        willSet {
            precondition(state == .stopped)
        }
    }
...
fileprivate extension MNMarqueeView {
    class TextLabel: UILabel {}
}

// MARK: - 滚动控制
extension MNMarqueeView {
...
fileprivate extension MNMarqueeView {
    ...
    class DisplayLinkTarget {
        ...
        weak var owner: MNMarqueeView?
        
        init(owner: MNMarqueeView) {
            self.owner = owner
        }
